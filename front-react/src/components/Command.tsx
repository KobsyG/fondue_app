...
const Command = ({ setOpenCommand, setOpenContact }: { setOpenCommand: React.Dispatch<React.SetStateAction<boolean>>, setOpenContact: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div style={{ fontFamily: 'OccamsEraser' }} className='contact-container relative h-[300px] w-[400px] rounded-[20px] flex items-center flex-col bg-[#202454]'>
      ...
      <a className='text-white border-2 p-2 text-[20px] my-auto lg:hover:scale-[1.10] lg:hover:bg-[#302450] transition-transform duration-200' href='https://www.morin-fromager.fr/produit/box-fondue-coeur' target='_blank' rel="noopener noreferrer">
        <h2>Morin Fromager</h2>
      </a>
      <a className='text-white border-2 p-2 text-[20px] my-auto lg:hover:scale-[1.10] lg:hover:bg-[#302450] transition-transform duration-200' href='https://www.bienmanger.com/1F48703_Box_Fondue_Coeur_Kit_Fondue_Fromagere_Auvergne_Pour_Personnes.html' target='_blank' rel="noopener noreferrer">
        <h2>bienmanger.com</h2>
      </a>
      <button onClick={() => setOpenContact(true)} className='my-auto flex items-center text-[17px] text-gray-400 lg:hover:scale-[1.05] transition-transform duration-200'>
